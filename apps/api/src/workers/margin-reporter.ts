...
export async function runMarginReporter(): Promise<void> {
  ...
  const rows: Array<{
    ...
    total_upstream_cost: string | null;
    total_user_charges: string | null;
  ...
  const report = {
    ...
    totals: {
      total_requests: rows.reduce((sum: number, r) => sum + (r.total_requests ?? 0), 0),
      total_upstream_cost: rows.reduce((sum: number, r) => sum + parseFloat(r.total_upstream_cost ?? "0"), 0),
      total_user_charges: rows.reduce((sum: number, r) => sum + parseFloat(r.total_user_charges ?? "0"), 0),
      total_margin: rows.reduce((sum: number, r) => sum + parseFloat(r.total_margin ?? "0"), 0),
    },
